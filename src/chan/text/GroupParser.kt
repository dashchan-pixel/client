...
@Public
class GroupParser private constructor(private val callback: Callback) {
	@Public
	...
	private class ReaderWrapper(val reader: Reader) {
		...
		@Throws(IOException::class)
		fun readTo(builder: StringBuilder?, stop: CharArray): Char {
			while (true) {
				val c = next()
				if (eof) {
					return (-1).toChar()
		...
		@Throws(IOException::class)
		fun skipTo(lowerCase: Boolean, end: String) {
			var index = 0
			while (true) {
				var c = next()
				if (eof) {
	...
	@Throws(ParseException::class)
	private fun onStartElement(tagName: String, source: StringBuilder, attrsStart: Int, attrsEnd: Int) {
	...
	@Throws(ParseException::class)
	private fun onEndElement(tagName: String, source: CharSequence) {
		if (groupTagName == null) {
