...
@Extendable
open class CommentEditor @Public constructor() {
	...
	class Tag(@JvmField val open: String, @JvmField val close: String, @JvmField val flags: Int)

	class FormatResult(@JvmField val start: Int, @JvmField val end: Int)
	...
	fun getTag(what: Int, close: Boolean): String? {
		val tag = tags.get(what) ?: return null
		return if (close) tag.close else tag.open
	}

	open fun getTag(what: Int, close: Boolean, length: Int): String? = getTag(what, close)
	...
	fun formatSelectedText(commentView: EditText, what: Int) {
	...
	open fun formatSelectedText(editable: Editable, what: Int, start: Int, end: Int): FormatResult? {
		val tag = tags.get(what) ?: return null
	...
	private fun formatSelectedTextOneLine(editable: Editable, what: Int, tag: Tag,
			startArg: Int, endArg: Int): FormatResult {
		var start = startArg
		var end = endArg
		var newStart = -1
		var newEnd = -1
	...
	@Extendable
	open class BulletinBoardCodeCommentEditor @Public constructor() : CommentEditor() {
		init {
	...
	@Extendable
	open class WakabaMarkCommentEditor @Public constructor() : CommentEditor() {
		init {
		...
		override fun getTag(what: Int, close: Boolean, length: Int): String? {
		...
		override fun formatSelectedText(editable: Editable, what: Int,
				start: Int, end: Int): FormatResult? {
		...
		override fun removeTags(comment: String?): String? {
			return super.removeTags(comment)?.replace("^H", "")
		}
