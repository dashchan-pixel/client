...
object StringUtils {
    ...
    @Public
    @JvmStatic
    fun emptyIfNull(string: CharSequence?): String = string?.toString().orEmpty()
    ...
    ): String? = replaceAll(string, Pattern.compile(regularExpression), replacementCallback)
    ...
    fun replaceAll(
        ...
        while (matcher.find()) {
            ...
            val replacement =
                replacementCallback.getReplacement(matcher)
    ...
    fun formatFileSize(
        ...
        val kb = size / 1000
        return if (kb >= 1000) {
            String.format(Locale.US, "%.1f", kb / 1000f) + " MB"
        } else {
            kb.toString() + if (upperCase) " KB" else " kB"
        }
    }

    @JvmStatic
    fun formatFileSizeMegabytes(size: Long): String = String.format(Locale.US, "%.2f", size / 1000f / 1000f) + " MB"
    ...
    @JvmStatic
    fun getNormalizedOriginalName(
        originalName: String?,
        fileName: String,
    ): String? = getNormalizedOriginalName(originalName, fileName, getFileExtension(fileName))

    private fun getNormalizedOriginalName(
        originalName: String?,
        fileName: String,
    ...
    @JvmStatic
    fun getNormalizedExtension(extension: String?): String? = C.EXTENSION_TRANSFORMATION[extension] ?: extension
    ...
    @JvmStatic
    fun formatBoardTitle(
        chanName: String,
        boardName: String?,
        title: String?,
    ): String =
        "/" + (if (boardName.isNullOrEmpty()) chanName else boardName) +
            if (title.isNullOrEmpty()) "/" else "/ — $title"

    @JvmStatic
    fun formatThreadTitle(
        chanName: String,
        boardName: String?,
        threadNumber: String,
    ): String = "/" + (if (boardName.isNullOrEmpty()) chanName else boardName) + "/" + threadNumber
