...
object CommonUtils {
	...
	@JvmStatic
	@JvmOverloads
	fun optJsonString(jsonObject: JSONObject, name: String, fallback: String? = null): String? {
		return try {
	...
	fun trimBitmap(bitmap: Bitmap?, backgroundColor: Int): Bitmap? {
		...
		if (actualLeft != 0 || actualTop != 0 || actualRight != width || actualBottom != height) {
			if (actualRight > actualLeft && actualBottom > actualTop) {
				val newBitmap = Bitmap.createBitmap(actualRight - actualLeft, actualBottom - actualTop,
	...
	@JvmStatic
	fun <T> removeNullItems(array: kotlin.Array<T?>?, itemClass: Class<T>): kotlin.Array<T?>? {
		if (array == null) {
			return null
		}
	...
	@JvmStatic
	fun <T> toArray(collection: Collection<T>?, itemClass: Class<T>): kotlin.Array<T>? {
