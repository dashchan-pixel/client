...
class Post : Comparable<Post> {
	...
	fun setAttachments(vararg attachments: Attachment?): Post {
		...
		builder!!.attachments = CommonUtils.removeNullItems(attachments as Array<Attachment?>,
	...
	fun setAttachments(attachments: Collection<Attachment>?): Post {
		@Suppress("UNCHECKED_CAST")
		builder!!.attachments = CommonUtils.toArray(attachments, Attachment::class.java)
				as Array<Attachment?>?
		return this
	}
	...
	@Public
	fun setVote(likes: Int, dislikes: Int): Post {
	...
	private class ChanBuilder {
		val builder = com.mishiranu.dashchan.content.model.Post.Builder()
			...
			for (attachment in attachments) {
				if (attachment is FileAttachment) {
					val file = com.mishiranu.dashchan.content.model.Post.Attachment.File
							.createExternal(attachment.fileUri, attachment.thumbnailUri,
									attachment.getOriginalName(), attachment.getSize(),
