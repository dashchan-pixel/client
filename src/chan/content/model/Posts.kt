...
class Posts {
    ...
    @Public
    fun getPosts(): Array<Post?>? = builder?.posts ?: provider?.getPosts()
    ...
    @Public
    fun setPosts(posts: Collection<Post>?): Posts = setPosts(*(CommonUtils.toArray(posts, Post::class.java) ?: emptyArray()))
    ...
    private class LazyProvider(
        val threadKey: PagesDatabase.ThreadKey,
    ) {
