...
class RedirectException : Exception {
	...
	class Target internal constructor(@JvmField val chanName: String?, @JvmField val boardName: String?,
			@JvmField val threadNumber: String?, @JvmField val postNumber: PostNumber?)
				...
				try {
					when {
						...
						chan.locator.isThreadUri(uri) -> {
							...
							val postNumber = if (postNumberString != null) {
	...
	companion object {
		...
		@JvmStatic
		@Public
		fun toBoard(boardName: String?): RedirectException {
			return RedirectException(boardName, null, null)
		}

		@JvmStatic
		@Public
		fun toThread(boardName: String?, threadNumber: String?, postNumber: String?): RedirectException {
			return toThread(boardName, threadNumber,
					if (postNumber != null) PostNumber.parseOrThrow(postNumber) else null)
		}

		@JvmStatic
		fun toThread(boardName: String?, threadNumber: String?, postNumber: PostNumber?): RedirectException {
			PostNumber.validateThreadNumber(threadNumber, false)
