...
class OverlineSpan {
	companion object {
		...
		@JvmStatic
		fun draw(textView: TextView, canvas: Canvas) {
			...
			if (spans != null && spans.isNotEmpty()) {
				...
				for (span in spans) {
					...
					for (i in lineStart..lineEnd) {
						val left = if (i == lineStart) layout.getPrimaryHorizontal(start)
						else layout.getLineLeft(i)
						val right = if (i == lineEnd) layout.getPrimaryHorizontal(end)
						else layout.getLineRight(i)
