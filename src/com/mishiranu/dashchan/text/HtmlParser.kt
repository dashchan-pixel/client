...
class HtmlParser<E, D, S : HtmlParser.SpanProvider<E>> private constructor(source: String,
		markup: Markup<E, D, S>?, private val parsingMode: Mode, private val threadNumber: String?,
		private val originalPostNumber: PostNumber?, private val extra: E?) : ContentHandler {
	interface Markup<E, D, S : SpanProvider<E>> {
		fun onBeforeTagStart(parser: HtmlParser<E, D, S>, builder: StringBuilder, tagName: String,
				attributes: Attributes, tagData: TagData): D?
		fun onTagStart(parser: HtmlParser<E, D, S>, builder: StringBuilder, tagName: String,
				attributes: Attributes, obj: D?)
		fun onTagEnd(parser: HtmlParser<E, D, S>, builder: StringBuilder, tagName: String)

		fun onListLineStart(parser: HtmlParser<E, D, S>, builder: StringBuilder,
				ordered: Boolean, line: Int): Int
		fun onCutBlock(parser: HtmlParser<E, D, S>, builder: StringBuilder)

		fun initSpanProvider(parser: HtmlParser<E, D, S>): S?
	}

	interface SpanProvider<E> {
		fun transformBuilder(parser: HtmlParser<E, *, *>, builder: StringBuilder): CharSequence?
	}

	class TagData(@JvmField var block: Boolean, @JvmField var spaced: Boolean, preformatted: Boolean) {
	...
	override fun startElement(uri: String?, tagName: String, qName: String?, attributes: Attributes) {
		...
		val preformattedTag = tagData.preformatted == TagData.Preformatted.ENABLED ||
		...
		lastBlock = if (blockTag) if (spacedTag) LAST_BLOCK_SPACED else LAST_BLOCK_COMMON else LAST_BLOCK_NONE
	...
	companion object {
		@JvmStatic
		fun <E, D, S : SpanProvider<E>> spanify(source: String?, markup: Markup<E, D, S>,
				threadNumber: String?, originalPostNumber: PostNumber?, extra: E?): CharSequence {
			return parse(source, markup, Mode.SPANIFY, threadNumber, originalPostNumber, extra)
		}

		@JvmStatic
		fun clear(source: String?): String {
			return parse<Nothing?, Nothing?, Nothing>(source, null, Mode.CLEAR, null, null, null).toString()
		}

		@JvmStatic
		fun <E, D, S : SpanProvider<E>> unmark(source: String?, markup: Markup<E, D, S>?, extra: E?): String {
			return parse(source, markup, Mode.UNMARK, null, null, extra).toString()
		}

		private fun <E, D, S : SpanProvider<E>> parse(source: String?, markup: Markup<E, D, S>?,
				parsingMode: Mode, threadNumber: String?, originalPostNumber: PostNumber?,
				extra: E?): CharSequence {
		...
		private val DEFAULT_TAGS = hashMapOf(
		...
		private val COLOR_PATTERN = Pattern.compile("color: ?(?:rgba?\\((\\d+), ?(\\d+)," +
				" ?(\\d+)(?:, ?\\d+)?\\)|(#[0-9A-Fa-f]+|[A-Za-z]+))")

		private val IDLE_MARKUP: Markup<*, *, *> = object : Markup<Any?, Any?, SpanProvider<Any?>> {
			override fun onBeforeTagStart(parser: HtmlParser<Any?, Any?, SpanProvider<Any?>>,
					builder: StringBuilder, tagName: String, attributes: Attributes,
					tagData: TagData): Any? = null

			override fun onTagStart(parser: HtmlParser<Any?, Any?, SpanProvider<Any?>>,
					builder: StringBuilder, tagName: String, attributes: Attributes, obj: Any?) {}

			override fun onTagEnd(parser: HtmlParser<Any?, Any?, SpanProvider<Any?>>,
					builder: StringBuilder, tagName: String) {}

			override fun onListLineStart(parser: HtmlParser<Any?, Any?, SpanProvider<Any?>>,
					builder: StringBuilder, ordered: Boolean, line: Int): Int = 0

			override fun onCutBlock(parser: HtmlParser<Any?, Any?, SpanProvider<Any?>>,
					builder: StringBuilder) {}

			override fun initSpanProvider(parser: HtmlParser<Any?, Any?, SpanProvider<Any?>>):
					SpanProvider<Any?>? = null
		}
	}
}
