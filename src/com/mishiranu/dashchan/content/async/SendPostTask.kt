...
class SendPostTask<Key>(
    private val key: Key,
    private val callback: Callback<Key>?,
    ...
    interface Callback<Key> {
        fun onSendPostChangeProgressState(
            key: Key,
        ...
        fun onSendPostChangeProgressValue(
            key: Key,
            progress: Long,
            progressMax: Long,
        )
    ...
    private fun switchProgressState(
        progressState: ProgressState,
        attachmentIndex: Int,
        force: Boolean,
    ) {
        if (lastProgressState != progressState || force) {
            ...
            callback?.onSendPostChangeProgressState(
                key,
                progressState,
                attachmentIndex,
            ...
            chanHolder.use().use {
                ...
                if (data.captchaNeedLoad) {
                    ...
                    if (data.captchaData != null) {
                        try {
                            fallbackHolder.use().use {
                                val response =
                                    ReadCaptchaTask.readForegroundCaptcha(
                                        fallbackHolder,
                                        chan.name,
                                        data.captchaData,
                                        data.captchaType,
                    ...
                    data.captchaData.put(
                        ChanPerformer.CaptchaData.INPUT,
                ...
                if (data.threadNumber == null && (result == null || result.threadNumber == null)) {
                    ...
                    val readThreadsResult =
                        try {
                            chan.performer.safe().onReadThreads(
                                ChanPerformer
                    ...
                    if (threads != null && threads.isNotEmpty()) {
                        ...
                        val wordsData1 = estimator.getWords<Unit>(postComment)
    ...
    override fun onProgress(progress: LongArray) {
        ...
        val progressValue = values[1]
        val progressMax = values[2]
        updateProgressValue(index, progressValue, progressMax)
    }
        ...
        if (callback != null) {
            if (result) {
                callback.onSendPostSuccess(
                    ...
                    data,
                    chan.name,
                    this.result?.threadNumber,
                    this.result?.postNumber,
                )
            } else {
