...
class ReadCaptchaTask(
    private val callback: Callback,
    captchaReader: CaptchaReader?,
    private val captchaType: String?,
    private val requirement: String?,
    private val captchaPass: List<String>?,
    private val mayShowLoadButton: Boolean,
    private val allowSolveAutomatically: Boolean,
    private val chan: Chan,
    private val boardName: String?,
    private val threadNumber: String?,
) : ExecutorTask<Unit, Pair<ErrorItem?, ReadCaptchaTask.Result?>?>() {
    private val chanHolder = HttpHolder(chan)
    private val fallbackHolder = HttpHolder(Chan.getFallback())

    /**
     * Whether this captcha is loaded for something the user is sending -- a post, a report, a vote
     * -- in which case it travels the same route as what it is solved for: one solved around a
     * sending-only proxy would be rejected.
     */
    var forSending: Boolean
    ...
    class Result(
        @JvmField val captchaState: CaptchaState?,
        @JvmField val captchaData: ChanPerformer.CaptchaData?,
        @JvmField val captchaType: String?,
        @JvmField val input: ChanConfiguration.Captcha.Input?,
        @JvmField val validity: ChanConfiguration.Captcha.Validity?,
    ...
    class RemoteResult(
        @JvmField val result: ChanPerformer.ReadCaptchaResult,
        @JvmField val challengeExtra: Any?,
        @JvmField val allowSolveAutomatically: Boolean,
    )
    ...
    override fun run(): Pair<ErrorItem?, Result?>? {
        ...
        try {
            chanHolder.use().use {
                result =
                    captchaReader.onReadCaptcha(
                        ChanPerformer.ReadCaptchaData(
                            captchaType,
                            @Suppress("UNCHECKED_CAST") (CommonUtils.toArray(captchaPass, String::class.java) as Array<String?>?),
                            mayShowLoadButton,
                            requirement,
                            boardName,
                            threadNumber,
            ...
            if (foregroundCaptcha != null) {
                try {
                    fallbackHolder.use().use {
                        captchaState =
                            if (CaptchaSolving.getInstance().checkActive(fallbackHolder)) {
                                CaptchaState.MAY_LOAD_SOLVING
                            } else {
                        ...
                        if (!mayShowLoadButton || invisible) {
                            val response =
                                readForegroundCaptcha(
                                    ...
                                    captchaData!!,
                                    foregroundCaptcha,
                                    result.challengeExtra,
                                    allowSolveAutomatically && result.allowSolveAutomatically,
                                )
    ...
    companion object {
        ...
        private fun checkForegroundCaptcha(captchaType: String?): ForegroundCaptcha? =
            when (StringUtils.emptyIfNull(captchaType)) {
        ...
        fun readForegroundCaptcha(
            holder: HttpHolder,
            chanName: String?,
            captchaData: ChanPerformer.CaptchaData,
            captchaType: String?,
        ): String? {
        ...
        private fun readForegroundCaptcha(
            holder: HttpHolder,
            captchaData: ChanPerformer.CaptchaData,
            foregroundCaptcha: ForegroundCaptcha,
            challengeExtra: Any?,
            ...
            if (recaptchaChallengeExtra == null) {
                recaptchaChallengeExtra =
                    if (foregroundCaptcha == ForegroundCaptcha.HCAPTCHA) {
                        recaptchaReader.getChallengeHcaptcha(
                            holder,
                            apiKey!!,
                            referer,
                            false,
