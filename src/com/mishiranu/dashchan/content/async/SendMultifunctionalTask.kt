...
class SendMultifunctionalTask(private val callback: Callback, private val state: State,
		private val type: String?, private val text: String?, options: List<String>?) :
		HttpHolderTask<Void, Boolean>(Chan.get(state.archiveChanName ?: state.chanName)) {
	...
	interface Callback {
		fun onSendSuccess(archiveBoardName: String?, archiveThreadNumber: String?)
		fun onSendFail(errorItem: ErrorItem?)
	}

	class State(@JvmField val operation: Operation, @JvmField val chanName: String?,
			@JvmField val boardName: String?, @JvmField val threadNumber: String?,
			@JvmField var types: List<Pair<String, String>>?,
			@JvmField var options: List<Pair<String, String>>?, @JvmField var commentField: Boolean) {
		...
		fun isArchiveSimpleQueryOnly(): Boolean {
			return archiveQueryOnly && options.isNullOrEmpty()
		}
	}
	...
	override fun run(holder: HttpHolder): Boolean {
		...
		try {
			when (state.operation) {
				Operation.DELETE -> {
					chan.performer.safe().onSendDeletePosts(ChanPerformer
				...
				Operation.REPORT -> {
					chan.performer.safe().onSendReportPosts(ChanPerformer
							.SendReportPostsData(state.boardName, state.threadNumber,
							createPostNumberList(state.postNumbers!!), type, options, text, holder))
				}
				Operation.VOTE -> {
					chan.performer.safe().onSendVotePost(ChanPerformer
							.SendVotePostData(state.boardName, state.threadNumber,
							state.postNumbers!![0].toString(), state.like, type, options, text, holder))
				}
				Operation.ARCHIVE -> {
					val uri = chan.locator.safe(false)
					...
					try {
						result = chan.performer.safe().onSendAddToArchive(ChanPerformer
