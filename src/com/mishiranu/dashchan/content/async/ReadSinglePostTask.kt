...
class ReadSinglePostTask(private val callback: Callback, private val chan: Chan,
		private val boardName: String?, private val threadNumber: String?,
		private val postNumber: PostNumber?) : HttpHolderTask<Void, Pair<ErrorItem?, PostItem?>>(chan) {
	interface Callback {
	...
	override fun run(holder: HttpHolder): Pair<ErrorItem?, PostItem?> {
		try {
			...
			val result = chan.performer.safe().onReadSinglePost(ChanPerformer
					.ReadSinglePostData(boardName, postNumber, holder))
			val post = result?.post ?: throw HttpException.createNotFoundException()
			return Pair(null, PostItem.createPost(post.post, chan,
