...
class ReadThreadSummariesTask(private val callback: Callback, private val chan: Chan,
	...
	interface Callback {
		fun onReadThreadSummariesSuccess(threadSummaries: List<ThreadSummary>, pageNumber: Int)
	...
	override fun run(holder: HttpHolder): List<ThreadSummary>? {
		try {
			val result = chan.performer.safe()
					.onReadThreadSummaries(ChanPerformer
	...
	companion object {
		@JvmStatic
		fun concatenate(threadSummaries1: List<ThreadSummary>?,
				threadSummaries2: List<ThreadSummary>?): List<ThreadSummary> {
			if (threadSummaries1 == null) {
				return threadSummaries2 ?: emptyList()
			...
			for (threadSummary in threadSummaries2) {
				if (!identifiers.contains(threadSummary.getBoardName() + '/' +
