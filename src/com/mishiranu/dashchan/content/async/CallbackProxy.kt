...
class CallbackProxy<Callback> private constructor(private val method: Method,
	...
	companion object {
		...
		@JvmStatic
		fun <Callback> create(callbackClass: Class<Callback>, handler: Handler<Callback>): Callback {
			val invocationHandler = InvocationHandler { proxy, method, args ->
				when (method.name) {
					"equals" -> args != null && args.size == 1 && args[0] === proxy
					"hashCode" -> handler.hashCode()
			...
			return Proxy.newProxyInstance(callbackClass.classLoader,
					arrayOf<Class<*>>(callbackClass), invocationHandler) as Callback
		}
	}
}
