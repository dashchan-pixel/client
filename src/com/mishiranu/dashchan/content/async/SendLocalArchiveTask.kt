...
class SendLocalArchiveTask(private val callback: Callback, private val chan: Chan,
	...
	override fun run(): Result? {
		...
		val htmlBuilder = WakabaLikeHtmlBuilder(posts.iterator().next().subject,
		...
		for (post in posts) {
			...
			for (span in spans) {
				...
				if (what != 0) {
					...
					if (what == ChanMarkup.TAG_SPECIAL_LINK) {
						...
						if (text.startsWith("&gt;&gt;")) {
							val uri = chan.locator.validateClickedUriString(decodeTo[1] as String?,
			...
			htmlBuilder.addPost(number.toString(), subject, name, identifier, tripcode, capcode, email,
					sage, originalPoster, timestamp, deleted, useDefaultName, htmlComment)
			for (icon in post.icons) {
				if (icon.uri != null && !StringUtils.isEmpty(icon.title)) {
					...
					if (downloadIcon && saveThumbnails) {
						thumbnailsToDownload.add(DownloadService.DownloadItem(chan.name,
			...
			for (attachment in post.attachments) {
				if (attachment is Post.Attachment.File) {
					...
					if (fileUri != null) {
						...
						htmlBuilder.addFile(filePath, thumbnailPath, originalName, attachment.size,
								attachment.width, attachment.height)
						if (saveFiles) {
							filesToDownload.add(DownloadService.DownloadItem(chan.name,
	...
	override fun onComplete(result: Result?) {
		...
		if (result != null) {
			...
			downloadResult = DownloadResult { binder ->
	...
	class Result(@JvmField val html: String, @JvmField val archiveName: String,
	...
	private fun replaceSpannable(spannable: SpannableStringBuilder, what: Char, with: String) {
		var i = 0
	...
	private fun makeSpanItem(what: Int, extra: Any?): SpanItem? {
		val openTag: String
		val closeTag: String
		when (what) {
			ChanMarkup.TAG_BOLD -> {
	...
	private fun getSpanType(span: Any, result: Array<Any?>): Array<Any?> {
		result[0] = 0
		result[1] = null
		if (span is LinkSpan) {
	...
	private fun chooseFileName(fileNamesLc: ArrayList<String>, fileName: String?): String? {
		var fileName = fileName
		if (fileName != null) {
			val locale = Locale.getDefault()
			val fileNameLc = fileName.lowercase(locale)
			if (fileNamesLc.contains(fileNameLc)) {
				...
				var i = 0
				do {
					newFileName = fileName + "-" + ++i + if (extension != null) ".$extension" else ""
	...
	companion object {
		...
		private fun createDownload(name: String, input: InputStream): DownloadResult {
			return DownloadResult { binder ->
				binder.downloadDirect(DataFile.Target.DOWNLOADS, DIRECTORY_ARCHIVE, name, input)
			}
		}

		private fun createDownload(path: String?,
				downloadItems: List<DownloadService.DownloadItem>): DownloadResult {
			return DownloadResult { binder ->
