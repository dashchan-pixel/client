...
class ReadVideoTask(private val callback: Callback, private val chan: Chan,
		private val uri: Uri, private val start: Long) : HttpHolderTask<LongArray, Boolean>(chan) {
	...
	override fun run(holder: HttpHolder): Boolean {
		...
		try {
			val result = chan.performer.safe()
					.onReadContent(ChanPerformer.ReadContentData(uri,
							CONNECT_TIMEOUT, READ_TIMEOUT, holder, if (start > 0) start else -1, -1))
