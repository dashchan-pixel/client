...
class ReadPostsTask(private val callback: Callback, private val chan: Chan,
		private val boardName: String?, private val threadNumber: String,
		private val loadFullThread: Boolean, pendingUserPosts: Collection<PendingUserPost>?) :
		HttpHolderTask<Void, ReadPostsTask.Result>(chan) {
	...
	interface Callback {
		...
		fun onReadPostsSuccess(cacheState: PagesDatabase.Cache.State?,
				replies: List<PagesDatabase.InsertResult.Reply>?, newCount: Int?)
		fun onReadPostsRedirect(target: RedirectException.Target)
		fun onReadPostsFail(errorItem: ErrorItem)
	}

	interface Result {
		class Success(@JvmField val cacheState: PagesDatabase.Cache.State?,
				@JvmField val removedPendingUserPosts: Set<PendingUserPost>?,
				@JvmField val replies: List<PagesDatabase.InsertResult.Reply>?,
				@JvmField val newCount: Int?) : Result

		class Redirect(@JvmField val target: RedirectException.Target) : Result

		class Fail(@JvmField val errorItem: ErrorItem) : Result
	}

	private class UpdateMeta(val deleted: Boolean, val error: Boolean) {
		fun isChanged(meta: PagesDatabase.Meta?): Boolean {
	...
	override fun run(holder: HttpHolder): Result {
		...
		try {
			...
			try {
				...
				val target = e.obtainTarget(chan.name)
						?: throw HttpException.createNotFoundException()
			...
			if (posts.isEmpty()) {
				if (partial) {
					...
					return Result.Success(PagesDatabase.getInstance().getCacheState(threadKey),
							null, emptyList(), null)
				} else {
					updateMeta = UpdateMeta(false, true)
			...
			if (pendingUserPosts != null && pendingUserPosts.isNotEmpty()) {
				...
				for (pendingUserPost in pendingUserPosts) {
					val postNumber = pendingUserPost.findUserPost(posts,
			...
			return Result.Success(insertResult.cacheState, removedPendingUserPosts,
			...
			if (responseCode == HttpURLConnection.HTTP_NOT_FOUND ||
				...
				if (chan.configuration.getOption(ChanConfiguration.OPTION_READ_SINGLE_POST)) {
					try {
						...
						val result = chan.performer.safe().onReadSinglePost(
								ChanPerformer.ReadSinglePostData(boardName, threadNumber, holder))
						val post = result?.post
						if (post != null) {
							val postThreadNumber = post.threadNumber
							if (threadNumber != postThreadNumber) {
								val target = RedirectException.toThread(boardName,
										postThreadNumber, post.post.number).obtainTarget(chan.name)!!
								updateMeta = UpdateMeta(true, false)
								return Result.Redirect(target)
							}
						}
	...
	override fun onComplete(result: Result) {
		when (result) {
			...
			is Result.Redirect -> callback.onReadPostsRedirect(result.target)
			is Result.Fail -> callback.onReadPostsFail(result.errorItem)
