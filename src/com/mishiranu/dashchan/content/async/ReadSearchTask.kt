...
class ReadSearchTask(
    private val callback: Callback,
    private val chan: Chan,
    private val boardName: String?,
    private val searchQuery: String?,
    val pageNumber: Int,
) : HttpHolderTask<Unit, List<PostItem>?>(chan) {
    private var errorItem: ErrorItem? = null

    interface Callback {
        fun onReadSearchSuccess(
    ...
    override fun run(holder: HttpHolder): List<PostItem>? {
        try {
            val result =
                chan.performer.safe().onReadSearchPosts(
                    ChanPerformer
    ...
    companion object {
        private val TIME_COMPARATOR =
            Comparator<SinglePost> { lhs, rhs ->
