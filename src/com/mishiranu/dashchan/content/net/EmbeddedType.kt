...
enum class EmbeddedType(
    ...
    YOUTUBE(
        ...
        1,
        listOf("youtu"),
        { locator, embeddedCode ->
            val fileUri =
                locator.buildQueryWithSchemeHost(
                    true,
                    "www.youtube.com",
                    "watch",
                    "v",
            ...
            val thumbnailUri =
                locator.buildPathWithSchemeHost(
                    true,
                    "img.youtube.com",
                    "vi",
                    embeddedCode,
            ...
            Post.Attachment.Embedded.createExternal(
                ...
                thumbnailUri,
                "YouTube",
                Post.Attachment.Embedded.ContentType.VIDEO,
                false,
                null,
            )
        },
    ...
    VIMEO(
        Pattern.compile("(?:https?://)(?:player\\.)?vimeo.com/(?:video/)?(?:channels/staffpicks/)?(\\d+)"),
        1,
        listOf("vimeo"),
        { locator, embeddedCode ->
    ...
    VOCAROO(
        ...
        { locator, embeddedCode ->
            ...
            Post.Attachment.Embedded.createExternal(
                ...
                null,
                "Vocaroo",
                Post.Attachment.Embedded.ContentType.AUDIO,
                true,
                forcedName,
            )
        },
    ...
    fun get(
        locator: ChanLocator,
        text: String?,
    ): String? = if (test(text)) locator.getGroupValue(text, pattern, index) else null

    fun getAll(
        locator: ChanLocator,
        text: String?,
    ): Array<String?>? = if (test(text)) locator.getUniqueGroupValues(text, pattern, index) else null

    fun obtainAttachment(
        locator: ChanLocator,
        embeddedCode: String,
    ): Post.Attachment.Embedded? = builder(locator, embeddedCode)
