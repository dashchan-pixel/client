...
class CloudFlareResolver : FirewallResolver() {
    ...
    override fun checkResponse(
        ...
        val pageBlockedByCloudflare =
            pageBlocked(response) &&
    ...
    private fun storeCookie(
        ...
        chan!!.configuration.storeCookie(
            key.formatKey(COOKIE_CLOUDFLARE)!!,
            cookie,
    ...
    private class CookieResult(
        val cookie: String,
        val uri: Uri,
    )

    private class Extra : WebViewExtra {
        override fun getInjectJavascript(): String? = IOUtils.readRawResourceString(MainApplication.getInstance().resources, R.raw.web_cloudflare_inject)

        companion object {
            @JvmField
            val CREATOR: Parcelable.Creator<Extra> =
                object : Parcelable.Creator<Extra> {
    ...
    private class WebViewClient : FirewallResolvers.WebViewClientWithExtra<CookieResult>("CloudFlare", Extra()) {
