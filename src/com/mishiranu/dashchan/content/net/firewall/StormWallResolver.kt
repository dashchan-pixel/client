...
class StormWallResolver : FirewallResolver() {
    ...
    private class WebViewClient : FirewallResolver.WebViewClient<CookieResult>("StormWall") {
        ...
        override fun onLoad(
            ...
            return path.isNullOrEmpty() || "/" == path || path == initialUri.getPath()
        }
    }

    private inner class Exclusive(
        val responseText: String,
    ) : FirewallResolver.Exclusive {
        ...
        override fun resolve(
            ...
            if (ceMatcher.find() && ckMatcher.find()) {
                ...
                if (ce.isNotEmpty() && ck != null) {
                    ...
                    val response =
                        HttpRequest(session.getUri(), session)
                            .setHeadMethod()
    ...
    private fun storeCookie(
        ...
        chan!!.configuration.storeCookie(
            key.formatKey(COOKIE_STORMWALL)!!,
            cookie,
    ...
    companion object {
        ...
        private fun toKey(session: FirewallResolver.Session): FirewallResolver.Exclusive.Key = session.getKey(FirewallResolver.Identifier.Flag.USER_AGENT)!!
    }
}
