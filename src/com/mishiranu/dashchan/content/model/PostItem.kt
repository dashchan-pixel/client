...
class PostItem private constructor(private val post: Post, threadDataBase: ThreadData.Base?,
	...
	enum class HideState(@JvmField val hidden: Boolean) {
		...
		class Map<T> {
			...
			fun get(key: T): HideState {
				val hidden = map[key]
				return if (hidden == null) UNDEFINED else if (hidden) HIDDEN else SHOWN
			}

			fun set(key: T, hideState: HideState) {
				when (hideState) {
					HIDDEN -> map[key] = true
					SHOWN -> map[key] = false
	...
	private class ThreadData(val base: Base, val commentShort: CharSequence,
			val commentShortSpans: Array<ColorScheme.Span>?, val gallerySet: GalleryItem.Set?) {
		class Base(val postsCount: Int, val filesCount: Int, val postsWithFilesCount: Int,
				val posts: List<Post>)
	}
	...
	init {
		...
		if (threadDataBase != null) {
			...
			threadData = null
			referencesTo = collectReferences(null, post.comment) ?: emptySet()
		}
	}
	...
	fun getBumpLimitReachedState(chan: Chan, postsCount: Int): BumpLimitState {
	...
	fun getComment(chan: Chan): CharSequence {
		...
		if (comment == null) {
			comment = obtainComment(post.comment, chan.markup,
			...
			linkSuffixSpans = (comment as? Spanned)?.getSpans(0, comment.length,
	...
	fun getComment(chan: Chan, repliesToPost: PostNumber): CharSequence {
	...
	fun getThreadCommentShort(maxWidth: Int, textSize: Float, maxLines: Int): CharSequence {
	...
	fun getAttachmentsDescription(resources: Resources,
			formatMode: AttachmentItem.FormatMode): String {
	...
	fun getReferencesFrom(): Set<PostNumber> = referencesFrom ?: emptySet()
	...
	fun formatThreadCardDescription(resources: Resources, repliesOnly: Boolean,
	...
	fun setHidden(hideState: HideState, hideReason: String?) {
	...
	companion object {
		...
		@JvmStatic
		fun createPost(post: Post, chan: Chan, boardName: String?, threadNumber: String?,
				originalPostNumber: PostNumber): PostItem {
			return PostItem(post, null, chan, boardName, threadNumber, originalPostNumber)
		}

		@JvmStatic
		fun createThread(posts: List<Post>, postsCount: Int, filesCount: Int,
				postsWithFilesCount: Int, chan: Chan, boardName: String?,
				threadNumber: String?): PostItem {
			val post = posts[0]
		...
		@JvmStatic
		fun collectReferences(references: MutableSet<PostNumber>?, comment: String?): Set<PostNumber>? {
				...
				while (true) {
					...
					if (index2 > index1++) {
						...
						if (start >= 0) {
							...
							val major = comment.substring(index1 + start,
									index1 + if (dotIndex >= 0) dotIndex else length).toInt()
							val minor = if (dotIndex >= 0) {
		...
		private fun obtainThreadComment(comment: String?, markup: ChanMarkup,
				extra: ChanMarkup.MarkupExtra): CharSequence {
			val commentBuilder = SpannableStringBuilder(obtainComment(comment,
