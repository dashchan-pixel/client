...
class Post private constructor(@JvmField val number: PostNumber, @JvmField val deleted: Boolean,
		private val flags: Int, @JvmField val timestamp: Long, @JvmField val subject: String,
		@JvmField val comment: String, @JvmField val commentMarkup: String, @JvmField val name: String,
		@JvmField val identifier: String, @JvmField val tripcode: String, @JvmField val capcode: String,
		@JvmField val email: String, @JvmField val attachments: List<Attachment>,
		@JvmField val icons: List<Icon>, @JvmField val vote: Vote?) : Comparable<Post> {
	interface Attachment {
		class File private constructor(@JvmField val fileUri: Uri?, @JvmField val thumbnailUri: Uri?,
				@JvmField val originalName: String, @JvmField val size: Int, @JvmField val width: Int,
				@JvmField val height: Int, @JvmField val spoiler: Boolean) : Attachment {
			companion object {
				@JvmStatic
				fun createExternal(fileUri: Uri?, thumbnailUri: Uri?, originalName: String?,
						size: Int, width: Int, height: Int, spoiler: Boolean): File? {
					return if (fileUri != null || thumbnailUri != null) {
						File(fileUri, thumbnailUri, StringUtils.emptyIfNull(originalName),
				...
				internal fun createInternal(fileUri: Uri?, thumbnailUri: Uri?, originalName: String,
						size: Int, width: Int, height: Int, spoiler: Boolean): File {
					return File(fileUri, thumbnailUri, originalName, size, width, height, spoiler)
				}
			}
		}

		class Embedded private constructor(@JvmField val fileUri: Uri?,
				@JvmField val thumbnailUri: Uri?, @JvmField val embeddedType: String,
				@JvmField val contentType: ContentType?, @JvmField val canDownload: Boolean,
				@JvmField val forcedName: String) : Attachment {
			enum class ContentType { AUDIO, VIDEO }

			companion object {
				internal fun validate(throwOnFail: Boolean, fileUri: Uri?, embeddedType: String?,
					...
					if (embeddedType.isNullOrEmpty()) {
						if (throwOnFail) {
							throw IllegalArgumentException("embeddedType is empty")
						}
						return false
					}
					if (contentType == null) {
						if (throwOnFail) {
							throw IllegalArgumentException("contentType is null")
						}
						return false
					}
					return true
				...
				@JvmStatic
				fun createExternal(throwOnFail: Boolean, fileUri: Uri?, thumbnailUri: Uri?,
						embeddedType: String?, contentType: ContentType?, canDownload: Boolean,
						forcedName: String?): Embedded? {
					if (!validate(throwOnFail, fileUri, embeddedType, contentType)) {
						return null
					}
					return Embedded(fileUri, thumbnailUri, StringUtils.emptyIfNull(embeddedType),
							contentType, canDownload, StringUtils.emptyIfNull(forcedName))
				}

				internal fun createInternal(fileUri: Uri?, thumbnailUri: Uri?, embeddedType: String,
						contentType: ContentType?, canDownload: Boolean, forcedName: String): Embedded {
					return Embedded(fileUri, thumbnailUri, embeddedType, contentType,
							canDownload, forcedName)
				}
			}
		}
	}

	class Icon internal constructor(@JvmField val uri: Uri?, @JvmField val title: String) {
		companion object {
			@JvmStatic
			fun createExternal(uri: Uri?, title: String?): Icon? {
				return if (uri != null && !title.isNullOrEmpty()) Icon(uri, title) else null
			}
		}
	}

	class Vote internal constructor(@JvmField val like: Int, @JvmField val dislike: Int) {
		@JvmField val showVotes: Boolean = true

		companion object {
			@JvmStatic
			fun createExternal(likes: Int, dislikes: Int): Vote = Vote(likes, dislikes)
		}
	}
	...
	class Builder {
		...
		fun build(deleted: Boolean): Post {
			val number = this.number ?: throw IllegalStateException("Post number is null")
			return Post(number, deleted, flags, timestamp,
					StringUtils.emptyIfNull(subject), StringUtils.emptyIfNull(comment),
					StringUtils.emptyIfNull(commentMarkup), StringUtils.emptyIfNull(name),
					StringUtils.emptyIfNull(identifier), StringUtils.emptyIfNull(tripcode),
					StringUtils.emptyIfNull(capcode), StringUtils.emptyIfNull(email),
					attachments ?: emptyList(), icons ?: emptyList(), vote)
		}
	}
	...
	companion object {
		...
		fun deserialize(number: PostNumber, deleted: Boolean, reader: JsonSerial.Reader): Post {
			var flags = 0
			var timestamp = 0L
			var subject = ""
			var comment = ""
						...
						while (!reader.endStruct()) {
							...
							while (!reader.endStruct()) {
								when (reader.nextName()) {
									...
									"fileUri" -> fileUri = Uri.parse(reader.nextString())
									"thumbnailUri" -> thumbnailUri = Uri.parse(reader.nextString())
									"originalName" -> originalName = reader.nextString()!!
									"size" -> size = reader.nextInt()
									"width" -> width = reader.nextInt()
									"height" -> height = reader.nextInt()
									"spoiler" -> spoiler = reader.nextBoolean()
									"embeddedType" -> embeddedType = reader.nextString()!!
									...
									"forcedName" -> forcedName = reader.nextString()!!
									else -> reader.skip()
								}
							}
							if ("file" == type) {
								list.add(Attachment.File.createInternal(fileUri, thumbnailUri,
										originalName, size, width, height, spoiler))
							} else if ("embedded" == type && Attachment.Embedded
											.validate(false, fileUri, embeddedType, contentType)) {
								list.add(Attachment.Embedded.createInternal(fileUri, thumbnailUri,
			...
			return Post(number, deleted, flags, timestamp, subject, comment, commentMarkup,
