...
interface PendingUserPost {
	fun findUserPost(posts: List<Post>, lastExistingPostNumber: PostNumber?,
			firstPostIsOriginal: Boolean): PostNumber?

	class NewThread private constructor() : PendingUserPost {
		override fun findUserPost(posts: List<Post>, lastExistingPostNumber: PostNumber?,
				firstPostIsOriginal: Boolean): PostNumber? {
			return if (firstPostIsOriginal && posts.isNotEmpty()) posts[0].number else null
		}
	...
	class SimilarComment(comment: String?, private val time: Long) : PendingUserPost {
		private val wordsData: SimilarTextEstimator.WordsData<Void>? = ESTIMATOR.getWords(comment)
		...
		override fun findUserPost(posts: List<Post>, lastExistingPostNumber: PostNumber?,
			...
			for (post in checkPosts) {
				...
				val wordsData = ESTIMATOR.getWords<Void>(comment)
		...
		override fun equals(other: Any?): Boolean {
			...
			if (other is SimilarComment) {
				return other.wordsData === wordsData || other.wordsData != null && wordsData != null &&
						other.wordsData.count == wordsData.count && other.wordsData.words == wordsData.words
			}
			return false
		}
