...
abstract class AttachmentItem protected constructor(private val master: Master) {
	...
	private class FileAttachmentItem(master: Master, locator: ChanLocator, fileUri: Uri?,
			thumbnailUri: Uri?, originalName: String?, private val fileSize: Int,
			val width: Int, val height: Int) : AttachmentItem(master) {
		val fileUri: Uri?
		...
		init {
			...
			this.thumbnailUri = if (C.IMAGE_EXTENSIONS.contains(extension) ||
					C.VIDEO_EXTENSIONS.contains(extension)) thumbnailUri else null
		...
		override fun getDialogTitle(chan: Chan): String? {
			return if (!normalizedOriginalName.isNullOrEmpty()) normalizedOriginalName else getFileName(chan)
		}
		...
		override fun createGalleryItem(): GalleryItem {
			return GalleryItem(fileUri, thumbnailUri, getBoardName(), getThreadNumber(),
					getPostNumber(), normalizedOriginalName, width, height, fileSize)
		}
	...
	private class EmbeddedAttachmentItem(master: Master, val fileUri: Uri, val thumbnailUri: Uri?,
			val embeddedType: String?, contentType: Post.Attachment.Embedded.ContentType?,
			val canDownload: Boolean, val fileName: String?,
			val fromComment: Boolean) : AttachmentItem(master) {
		...
		override fun getType(): Type = if (isAudio) Type.AUDIO else if (isVideo) Type.VIDEO else Type.FILE

		override fun getGeneralType(): GeneralType {
			return if (fromComment) GeneralType.LINK else GeneralType.FILE
		}
		...
		override fun getDescription(formatMode: FormatMode): String {
			...
			if (formatMode == FormatMode.LONG || formatMode == FormatMode.TWO_LINES ||
				...
				builder.append(if (formatMode == FormatMode.TWO_LINES ||
						formatMode == FormatMode.THREE_LINES) '\n' else ' ')
			}
	...
	fun configureAndLoad(view: AttachmentView, chan: Chan, needShowMultipleIcon: Boolean, force: Boolean) {
		...
		val overlay = if (needShowMultipleIcon) {
			...
			when (type) {
				Type.IMAGE -> if (key.isNullOrEmpty()) AttachmentView.Overlay.WARNING
				else AttachmentView.Overlay.NONE
				Type.VIDEO -> AttachmentView.Overlay.VIDEO
				Type.AUDIO -> AttachmentView.Overlay.AUDIO
				Type.FILE -> AttachmentView.Overlay.FILE
			}
		}
	...
	fun startLoad(view: AttachmentView, chan: Chan, force: Boolean) {
	...
	fun canLoadThumbnailManually(attachmentView: AttachmentView, chan: Chan): Boolean {
	...
	companion object {
		@JvmStatic
		fun obtain(master: Master, post: Post, locator: ChanLocator): ArrayList<AttachmentItem>? {
			val attachmentItems = ArrayList<AttachmentItem>()
			for (attachment in post.attachments) {
				val attachmentItem: AttachmentItem? = when (attachment) {
					is Post.Attachment.File -> obtainFileAttachmentItem(master, locator, attachment)
		...
		private fun obtainFileAttachmentItem(master: Master, locator: ChanLocator,
				file: Post.Attachment.File?): AttachmentItem? {
			if (file == null) {
				return null
			}
			...
			return FileAttachmentItem(master, locator, fileUri, thumbnailUri, file.originalName,
					file.size, file.width, file.height)
		}

		private fun obtainEmbeddedAttachmentItem(master: Master, locator: ChanLocator,
				embedded: Post.Attachment.Embedded?, fromComment: Boolean): AttachmentItem? {
			if (embedded == null) {
				return null
			}
			...
			return EmbeddedAttachmentItem(master, fileUri, embedded.thumbnailUri, embedded.embeddedType,
					embedded.contentType, canDownload, fileName, fromComment)
		}

		private fun addCommentAttachmentItems(attachmentItems: MutableList<AttachmentItem>,
				master: Master, locator: ChanLocator, comment: String?, embeddedType: EmbeddedType) {
