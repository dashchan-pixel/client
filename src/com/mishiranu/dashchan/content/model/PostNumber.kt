...
class PostNumber(
...
) : Comparable<PostNumber>,
    Parcelable {
    override fun toString(): String = if (minor != 0) "$major.$minor" else major.toString()
    ...
    companion object {
        ...
        fun parseOrThrow(postNumber: String?): PostNumber =
            parseNullable(postNumber) ?: throw IllegalArgumentException(
                "Post number is not valid: $postNumber. Post number must be a positive number " +
                    "or a pair of positive numbers separated by dot.",
            )

        @JvmStatic
        fun validateThreadNumber(
            threadNumber: String?,
            allowEmpty: Boolean,
        ) {
            if (threadNumber.isNullOrEmpty()) {
                require(allowEmpty) { "Thread number is not defined" }
            } else {
                val escapedThreadNumber = StringUtils.escapeFile(threadNumber, false)
                require(threadNumber.length <= 30 && escapedThreadNumber == threadNumber) {
                    "Thread number is not valid: $threadNumber. " +
        ...
        @JvmField
        val CREATOR =
            object : Parcelable.Creator<PostNumber> {
