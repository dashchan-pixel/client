...
object Preferences {
    ...
    const val KEY_POSTING_CLASSIC: String = "posting_classic"
    const val DEFAULT_POSTING_CLASSIC: Boolean = false

    /**
     * Whether the submission form is shown as a draggable bottom sheet
     * ([com.mishiranu.dashchan.widget.PostingSheetLayout]) instead of filling the screen. It is, unless
     * *Classic style* has been turned on, which is the one thing [KEY_POSTING_CLASSIC] says.
     */
    @JvmStatic
    ...
    /**
     * Whether the proxy is restricted to what the user sends: read-only browsing goes out directly,
     * and everything the user hands the forum — a post, a report, a deletion, a vote, a thread
     * given to an archive, a passcode checked, and the captchas any of them are asked for — plus
     * the checks that exist to verify the proxy itself are routed through it. Has no effect when no
     * proxy is configured.
     */
