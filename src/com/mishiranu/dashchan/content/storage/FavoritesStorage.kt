...
class FavoritesStorage private constructor() : StorageManager.Storage<List<FavoritesStorage.FavoriteItem>>("favorites", 2000, 10000) {
    ...
    fun canSortManually(): Boolean = Preferences.favoritesOrder != Preferences.FavoritesOrder.TITLE

    private fun notifyFavoritesUpdate(
        favoriteItem: FavoriteItem,
        action: Action,
    ) {
    ...
    fun getFavorite(
        ...
        boardName: String?,
        threadNumber: String?,
    ): FavoriteItem? = favoriteItemsMap[makeKey(chanName, boardName, threadNumber)]

    fun hasFavorite(
        chanName: String?,
        boardName: String?,
        threadNumber: String?,
    ): Boolean = getFavorite(chanName, boardName, threadNumber) != null
    ...
    fun moveAfter(
        favoriteItem: FavoriteItem,
        afterFavoriteItem: FavoriteItem?,
    ) {
            ...
            if (changed) {
                notifyFavoritesUpdate(
                    ...
                    if (favoriteItem.watcherEnabled) {
                        Action.WATCHER_ENABLE
                    } else {
                        Action.WATCHER_DISABLE
                    },
                )
                serialize()
            }
    ...
    fun getThreads(chanName: String?): ArrayList<FavoriteItem> = getFavorites(chanName, threads = true, boards = false, orderByBoardName = false)

    fun getBoards(chanName: String?): ArrayList<FavoriteItem> = getFavorites(chanName, threads = false, boards = true, orderByBoardName = true)

    private fun getFavorites(
        chanName: String?,
        threads: Boolean,
        boards: Boolean,
        orderByBoardName: Boolean,
    ): ArrayList<FavoriteItem> {
    ...
    private val chanNameIndexAscendingComparator =
        Comparator<FavoriteItem> { lhs, rhs ->
    ...
    private val identifiersComparator =
        Comparator<FavoriteItem> { lhs, rhs ->
    ...
    companion object {
        ...
        private fun makeKey(favoriteItem: FavoriteItem): String = makeKey(favoriteItem.chanName, favoriteItem.boardName, favoriteItem.threadNumber)

        private fun compareChanNames(
            lhs: FavoriteItem,
            rhs: FavoriteItem,
        ): Int = ChanManager.getInstance().compareChanNames(lhs.chanName, rhs.chanName)

        private fun compareBoardNames(
            lhs: FavoriteItem,
            rhs: FavoriteItem,
        ): Int = StringUtils.compare(lhs.boardName, rhs.boardName, false)

        private fun compareThreadNumbers(
            lhs: FavoriteItem,
            rhs: FavoriteItem,
        ): Int = StringUtils.compare(lhs.threadNumber, rhs.threadNumber, false)
    }
}
