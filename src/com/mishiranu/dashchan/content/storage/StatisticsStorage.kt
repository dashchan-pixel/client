...
class StatisticsStorage private constructor() :
		StorageManager.Storage<Map<String, StatisticsStorage.StatisticsItem>>(
				"statistics", 1000, 10000) {
				...
				when (reader.nextName()) {
					KEY_DATA -> {
						...
						while (!reader.endStruct()) {
							...
							if (chanName != null) {
								statisticsItems[chanName] = StatisticsItem(threadsViewed,
										postsSent, threadsCreated)
							}
						}
					}
					KEY_START_TIME -> startTime = maxOf(reader.nextLong(), 0L)
					else -> reader.skip()
				}
			}
	...
	@Throws(IOException::class)
	override fun onWrite(data: Map<String, StatisticsItem>, output: OutputStream) {
	...
	class StatisticsItem internal constructor(@JvmField var threadsViewed: Int,
			@JvmField var postsSent: Int, @JvmField var threadsCreated: Int)
	...
	fun incrementPostsSent(chanName: String, newThread: Boolean) {
		val chan = Chan.get(chanName)
		val statistics = chan.configuration.safe().obtainStatistics()
