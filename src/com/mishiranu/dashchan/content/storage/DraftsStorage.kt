...
class DraftsStorage private constructor() :
		StorageManager.Storage<Pair<List<DraftsStorage.PostDraft>, List<DraftsStorage.AttachmentDraft>>>(
				"drafts", 2000, 10000) {
	...
	override fun onClone(): Pair<List<PostDraft>, List<AttachmentDraft>> {
		return Pair(ArrayList(postDrafts.values), ArrayList(futureAttachmentDrafts))
	}
	...
	@Throws(IOException::class)
	override fun onWrite(data: Pair<List<PostDraft>, List<AttachmentDraft>>, output: OutputStream) {
	...
	fun getPostDraft(chanName: String?, boardName: String?, threadNumber: String?): PostDraft? {
		return postDrafts[makeKey(chanName, boardName, threadNumber)]
	}

	fun removePostDraft(chanName: String?, boardName: String?, threadNumber: String?) {
	...
	fun store(chanName: String?, captchaDraft: CaptchaDraft?) {
	...
	fun getCaptchaDraft(chanName: String?): CaptchaDraft? {
		...
		if (captchaDraft != null && captchaChanName == chanName) {
			...
			if (captcha != null && !captcha.hasLifetime()) {
				...
				val minutesSinceCaptchaCreation = TimeUnit.MILLISECONDS
	...
	fun storeFuture(fileHolder: FileHolder): Boolean {
		...
		if (hash != null) {
			val attachmentDraft = AttachmentDraft(hash, fileHolder.name, null,
	...
	class PostDraft(@JvmField val chanName: String?, @JvmField val boardName: String?,
			@JvmField val threadNumber: String?, @JvmField val name: String?,
			@JvmField val email: String?, @JvmField val password: String?,
			@JvmField val subject: String?, @JvmField val comment: String?,
			@JvmField val commentCarriage: Int,
			@JvmField val attachmentDrafts: ArrayList<AttachmentDraft>?,
			@JvmField val optionSage: Boolean, @JvmField val optionSpoiler: Boolean,
			@JvmField val optionOriginalPoster: Boolean, @JvmField val userIcon: String?) {
		constructor(chanName: String?, boardName: String?, threadNumber: String?,
				name: String?, email: String?, password: String?,
				optionSage: Boolean, optionOriginalPoster: Boolean, userIcon: String?) :
				this(chanName, boardName, threadNumber, name, email, password, null, null, 0,
						null, optionSage, false, optionOriginalPoster, userIcon)

		val isEmpty: Boolean
			get() = StringUtils.isEmpty(name) && StringUtils.isEmpty(email) &&
					StringUtils.isEmpty(password) && StringUtils.isEmpty(subject) &&
					StringUtils.isEmpty(comment) && attachmentDrafts.isNullOrEmpty() &&
			...
			fun deserialize(reader: JsonSerial.Reader): PostDraft {
				...
				while (!reader.endStruct()) {
					when (reader.nextName()) {
						...
						KEY_EMAIL -> email = reader.nextString()
						KEY_PASSWORD -> password = reader.nextString()
						KEY_SUBJECT -> subject = reader.nextString()
						KEY_COMMENT -> comment = reader.nextString()
						KEY_COMMENT_CARRIAGE -> commentCarriage = reader.nextInt()
						...
						KEY_OPTION_SAGE -> optionSage = reader.nextBoolean()
						KEY_OPTION_SPOILER -> optionSpoiler = reader.nextBoolean()
						KEY_OPTION_ORIGINAL_POSTER -> optionOriginalPoster = reader.nextBoolean()
						KEY_USER_ICON -> userIcon = reader.nextString()
						else -> reader.skip()
					}
				}
				return PostDraft(chanName, boardName, threadNumber, name, email, password,
	...
	class CaptchaDraft(@JvmField val captchaType: String?,
			...
			@JvmField val text: String?, @JvmField val captcha: CaptchaForm.Captcha?,
			@JvmField val large: Boolean, @JvmField val blackAndWhite: Boolean,
			@JvmField val boardName: String?, @JvmField val threadNumber: String?) : Parcelable {
		...
		companion object {
			@JvmField
			val CREATOR: Parcelable.Creator<CaptchaDraft> = object : Parcelable.Creator<CaptchaDraft> {
				override fun createFromParcel(source: Parcel): CaptchaDraft {
					val captchaType = source.readString()
					val captchaState = source.readString()
							?.let { ReadCaptchaTask.CaptchaState.valueOf(it) }
					val captchaData = if (source.readByte().toInt() != 0)
						ChanPerformer.CaptchaData.CREATOR.createFromParcel(source) else null
					val loadedCaptchaType = source.readString()
					val loadedInput = source.readString()
					...
					return CaptchaDraft(captchaType, captchaState, captchaData, loadedCaptchaType,
							loadedInput, loadedValidity, text, captcha, large, blackAndWhite,
	...
	class AttachmentDraft(@JvmField val hash: String, @JvmField val name: String?,
			@JvmField val newname: String?, @JvmField val rating: String?,
			@JvmField val optionUniqueHash: Boolean, @JvmField val optionRemoveMetadata: Boolean,
			@JvmField val optionRemoveFileName: Boolean, @JvmField val optionSpoiler: Boolean,
		...
		companion object {
			...
			fun deserialize(reader: JsonSerial.Reader): AttachmentDraft? {
				...
				while (!reader.endStruct()) {
					when (reader.nextName()) {
						KEY_HASH -> hash = reader.nextString()
						KEY_NAME -> name = reader.nextString()
						KEY_NEWNAME -> newname = reader.nextString()
						KEY_RATING -> rating = reader.nextString()
						KEY_OPTION_UNIQUE_HASH -> optionUniqueHash = reader.nextBoolean()
						KEY_OPTION_REMOVE_METADATA -> optionRemoveMetadata = reader.nextBoolean()
						KEY_OPTION_REMOVE_FILE_NAME -> optionRemoveFileName = reader.nextBoolean()
						KEY_OPTION_SPOILER -> optionSpoiler = reader.nextBoolean()
						KEY_OPTION_NEW_FILENAME -> optionCustomName = reader.nextBoolean()
						KEY_REENCODING -> {
				...
				return AttachmentDraft(hash!!, name, newname, rating, optionUniqueHash,
						optionRemoveMetadata, optionRemoveFileName, optionSpoiler,
						reencoding, optionCustomName)
			}
		}
	}
	...
	companion object {
		...
		private fun makeKey(postDraft: PostDraft): String {
