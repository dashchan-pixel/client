...
class ThreadsDatabase internal constructor(
    ...
    class StateExtra(
        @JvmField val state: ByteArray?,
        @JvmField val extra: ByteArray?,
    ) {
        companion object {
    ...
    override fun open(database: SQLiteDatabase) {
        val clean: Boolean
        database
            .rawQuery("SELECT COUNT(*) FROM " + Schema.Threads.Companion.TABLE_NAME, null)
            .use { cursor ->
                clean =
                    cursor.moveToFirst() &&
                    cursor.getInt(0) > Schema.Threads.Companion.MAX_COUNT
            }
        if (clean) {
            val time: Long?
            ...
            database
                .query(
                    ...
                    null,
                    null,
                    Schema.Threads.Columns.Companion.TIME + " DESC",
                    (Schema.Threads.Companion.MAX_COUNT_FACTOR * Schema.Threads.Companion.MAX_COUNT)
                        .toInt()
                        .toString() + ", 1",
                ).use { cursor ->
    ...
    internal fun getFlags(
        ...
        hiddenThreads: HideState.Map<String>,
    ) {
        Objects.requireNonNull<String?>(chanName)
        var update = false
        val projection =
    ...
    fun setStateExtra(
        ...
        if (async) {
            ...
            database.execute<Unit?>(callback)
        }
    }
