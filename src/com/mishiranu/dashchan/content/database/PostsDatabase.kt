...
class PostsDatabase internal constructor(
    ...
    class Flags(
        @JvmField val hiddenPosts: HideState.Map<PostNumber?>?,
        @JvmField val userPosts: HashSet<PostNumber>?,
    )
    ...
    override fun open(database: SQLiteDatabase) {
        val clean: Boolean
        database
            .rawQuery("SELECT COUNT(*) FROM " + Schema.Posts.Companion.TABLE_NAME, null)
            .use { cursor ->
        ...
        if (clean) {
            ...
            database
                .query(
                    ...
                    null,
                    null,
                    Schema.Posts.Columns.Companion.TIME + " DESC",
                    (Schema.Posts.Companion.MAX_COUNT_FACTOR * Schema.Posts.Companion.MAX_COUNT)
                        .toInt()
                        .toString() + ", 1",
                ).use { cursor ->
    ...
    fun setFlagsMigration(
        ...
        boardName: String?,
        threadNumber: String,
        flagsMap: MutableMap<PostNumber?, Pair<HideState?, Boolean?>?>,
        time: Long,
    ) {
