...
class JpegData private constructor(@JvmField val exifData: ExifData?,
	...
	companion object {
		...
		fun extract(fileInput: InputStream): JpegData {
			...
			val forbidRegionDecoder = sofBytes != null && sofBytes.size > 7 &&
					sofBytes[5].toInt() and 0xff == 1 && sofBytes[7].toInt() and 0xff != 0x11
			val exifData = if (exifBytes != null) ExifData.extract(exifBytes, 0) else null
			return JpegData(if (exifData == null && exifBytes != null) ExifData.EMPTY else exifData,
					forbidRegionDecoder)
		}
	}
}
