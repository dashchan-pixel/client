...
class ExifData private constructor(
    private val exif: Map<String, String?>?,
) {
    @Volatile
    ...
    fun mergeTo(exifData: ExifData?): ExifData =
        if (exifData?.exif == null || exifData.exif.isEmpty()) {
            this
        } else if (exif.isNullOrEmpty()) {
            exifData
        } else {
    ...
    companion object {
        ...
        private fun add(
            userMetadata: MutableList<Pair<String, String>?>,
        ...
        private fun convertIfdRational(
            exifBytes: ByteArray,
            offset: Int,
            format: Int,
        ...
        private fun convertIfdGpsString(
            exifBytes: ByteArray,
            offset: Int,
            format: Int,
            littleEndian: Boolean,
        ...
        private fun formatDoubleSimple(value: Double): String = StringUtils.stripTrailingZeros(String.format(Locale.US, "%.1f", value))

        private fun extractIfd(
            exif: MutableMap<String, String?>,
            ifd: Ifd,
            exifBytes: ByteArray,
            offset: Int,
            littleEndian: Boolean,
                        ...
                        if (ifd == Ifd.GENERAL) {
                            when (type) {
                                ...
                                EXIF_TAG_ORIENTATION -> {
                                    exif[KEY_ORIENTATION] = valueShort.toString()
                                }
                                ...
                                EXIF_TAG_EXIF_IFD -> {
                                    exif[KEY_EXIF_OFFSET] = value.toString()
                                }

                                EXIF_TAG_GPS_IFD -> {
                                    exif[KEY_GPS_OFFSET] = value.toString()
                                }

                                EXIF_TAG_ISO_SPEED -> {
                                    exif[KEY_ISO_SPEED] = valueShort.toString()
                                }
                            ...
                            when (type) {
                                0x0001 -> {
                                    exif[KEY_LATITUDE_REF] = value.toChar().toString()
                                }
                                ...
                                0x0003 -> {
                                    exif[KEY_LONGITUDE_REF] = value.toChar().toString()
                                }
