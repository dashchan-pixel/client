...
class ErrorEditTextSetter(private val editText: EditText) {
	...
	fun setError(error: Boolean) {
		if (this.error != error) {
			...
			if (backgroundNormal == null) {
				backgroundNormal = ResourceUtils.getDrawable(editText.context,
						android.R.attr.editTextBackground, 0)
				val backgroundError = ResourceUtils.getDrawable(editText.context,
						android.R.attr.editTextBackground, 0)
				this.backgroundError = backgroundError
				if (colorFilter == null) {
					colorFilter = PorterDuffColorFilter(ResourceUtils.getColor(editText.context,
