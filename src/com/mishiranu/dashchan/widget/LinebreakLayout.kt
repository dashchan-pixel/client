...
class LinebreakLayout : ViewGroup {
    ...
    private fun measureChild(
        ...
        val childWidthMeasureSpec =
            ...
            } else if (widthUnspecified) {
                MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED)
            } else {
                MeasureSpec.makeMeasureSpec(
                    Math.max(
                        maxWidth - (if (lineWidth > 0) lineWidth + horizontalSpacing else 0),
                        0,
                    ),
                    if (linebreak) MeasureSpec.EXACTLY else MeasureSpec.AT_MOST,
                )
            }
        val childHeightMeasureSpec =
            if (matchLineHeight == 0) {
                getChildMeasureSpec(heightMeasureSpec, vertialPaddings, layoutParams.height)
            } else {
                MeasureSpec.makeMeasureSpec(matchLineHeight, MeasureSpec.EXACTLY)
            }
