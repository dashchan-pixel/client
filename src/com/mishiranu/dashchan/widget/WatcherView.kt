...
class WatcherView(context: Context, private val colorSet: ColorSet) : FrameLayout(context) {
	class ColorSet(@JvmField val enabledColor: Int, @JvmField val unavailableColor: Int,
	...
	override fun draw(canvas: Canvas) {
		...
		rectF.set(paddingHorizontal.toFloat(), paddingVertical.toFloat(), (width - paddingHorizontal).toFloat(),
	...
	fun update(counter: WatcherService.Counter) {
		...
		if (counter.newCount <= 0 && counter.deleted) {
			...
			var value = if (abs(counter.newCount) >= 1000) {
