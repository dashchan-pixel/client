...
class SummaryLayout(
    dialog: AlertDialog,
) : CommentTextView.PrepareToCopyListener,
    ...
    fun add(
        ...
        if (builder.length > 0) {
            ...
            if (addDivider && dividerDrawable != null) {
                builder.append("  \n")
                builder.setSpan(
                    PaddingSpan(dividerPaddingTextSize),
                    builder.length - 2,
                    builder.length,
                    SpannableStringBuilder.SPAN_EXCLUSIVE_EXCLUSIVE,
                )
                builder.setSpan(
                    DividerSpan(textView, dividerDrawable, dividerExtraTop, dividerExtraBottom),
                    builder.length - 3,
                    builder.length - 2,
                    SpannableStringBuilder.SPAN_EXCLUSIVE_EXCLUSIVE,
                )
                ...
                builder.append('\n')
                builder.setSpan(
                    PaddingSpan(blockPaddingTextSize),
                    builder.length - 1,
                    builder.length,
                    SpannableStringBuilder.SPAN_EXCLUSIVE_EXCLUSIVE,
                )
        ...
        builder.setSpan(
            RelativeSizeSpan(TITLE_SIZE_SP.toFloat() / TEXT_SIZE_SP),
        ...
        builder.append('\n')
        builder.setSpan(
            PaddingSpan(titlePaddingTextSize),
            builder.length - 1,
            builder.length,
            SpannableStringBuilder.SPAN_EXCLUSIVE_EXCLUSIVE,
        )
    ...
    private class DividerSpan(
        ...
        private val extraTop: Int,
        private val extraBottom: Int,
    ) : ReplacementSpan() {
        ...
        private fun addTestLines(
            ...
            if (titleFirst) {
                builder.setSpan(
                    ...
                    builder.length - lineText.length - 1,
                    builder.length,
                    SpannableStringBuilder.SPAN_EXCLUSIVE_EXCLUSIVE,
                )
            }
            if (titleSecond) {
                builder.setSpan(
                    RelativeSizeSpan(titleSize),
                    builder.length - 2 * lineText.length - 2,
                    builder.length - lineText.length - 1,
                    SpannableStringBuilder.SPAN_EXCLUSIVE_EXCLUSIVE,
                )
            }
