    ...
    constructor(
        ...
        interface LinkListener {
            class Extra(
                ...
                @JvmField val inBoardLink: Boolean,
            ) {
                companion object {
                    @JvmField
        ...
        class ExtraButton(
            ...
            class Text(
                ...
                override fun toString(): String = if (end > start) text.toString().substring(start, end) else ""
            }
        ...
        override fun setCustomSelectionActionModeCallback(actionModeCallback: ActionMode.Callback?): Unit = throw UnsupportedOperationException()

        override fun setTextIsSelectable(selectable: Boolean): Unit = throw UnsupportedOperationException()

        fun isSelectionMode(): Boolean = selectionMode != null
        ...
        private fun sendFakeMotionEvent(
            action: Int,
            x: Int,
            y: Int,
        ) {
        ...
        private fun startSelection(
            ...
            if (posX != Int.MAX_VALUE &&
                posY != Int.MAX_VALUE &&
                (startOffset < 0 || endOffset < 0 || endOffset > length || startOffset >= endOffset)
            ) {
                startOffset = 0
                endOffset = text.length
                val lx = posX - getTotalPaddingLeft()
                val ly = posY - getTotalPaddingTop()
                if (lx >= 0 && ly >= 0 && lx < getWidth() - getTotalPaddingRight() && ly < getHeight() - getTotalPaddingBottom()) {
                    ...
                    if (offset >= 0 && offset < length) {
                        ...
                        if (endOffset > startOffset) {
                            val part = text.subSequence(startOffset, endOffset).toString()
            ...
            posX = getTotalPaddingLeft()
            posY = getTotalPaddingRight()
            setSelectionMode(SelectionMode.INITIAL)
            val finalX = posX
            val finalY = posY
            val finalStart = startOffset
            val finalEnd = endOffset
            post(
                Runnable {
        ...
        private fun getPartialCommentString(
            text: Spannable,
            start: Int,
            end: Int,
        ): String? {
        ...
        private fun onCreateSelectionMenu(
            menu: Menu,
            order: Int,
        ) {
            for (i in EXTRA_BUTTON_IDS.indices) {
                ...
                if (extraButton != null) {
                    menu
                        .add(0, EXTRA_BUTTON_IDS[i], order, extraButton.title)
        ...
        override fun onTextContextMenuItem(id: Int): Boolean = super.onTextContextMenuItem(id)
        ...
        override fun setMaxLines(maxLines: Int): Unit = throw UnsupportedOperationException()
        ...
        override fun hasExplicitFocusable(): Boolean = super.hasExplicitFocusable() && isSelectionMode()

        override fun hasFocusable(): Boolean = super.hasFocusable() && isSelectionMode()
        ...
        private fun fillLayoutPosition(
            x: Float,
            y: Float,
        ): Point {
            var ix = x.toInt()
        ...
        private fun checkAcceptSpan(
            x: Float,
            y: Float,
            layoutPosition: Point?,
        ): Boolean {
            var currentLayoutPosition = layoutPosition
            val insideTouchSlop = abs(x - spanStartX) <= touchSlop && abs(y - spanStartY) <= touchSlop
            if (insideTouchSlop) {
                return true
            }
            if (currentLayoutPosition == null) {
                currentLayoutPosition = fillLayoutPosition(x, y)
            }
            val layout = getLayout()
            val text = this.spannedText
            if (layout != null && text != null) {
                val spans = findSpansToClick(layout, text, Any::class.java, currentLayoutPosition)
                for (span in spans) {
                    if (span === spanToClick) {
                        return true
                    }
        ...
        private val linkLongClickRunnable =
            Runnable {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (layout != null && text != null) {
                    ...
                    if (!linkSpans.isEmpty()) {
                        setSpanToClick(linkSpans[0], x, y)
                        postDelayed(
