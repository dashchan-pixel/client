...
class EdgeEffectHandler private constructor(context: Context, shift: Shift?) {
	...
	private class ControlledEdgeEffect(context: Context, private val shift: Shift, private val side: Side) :
			EdgeEffect(context) {
	...
	init {
		val shift = shift ?: Shift { 0 }
		topEdgeEffect = ControlledEdgeEffect(context, shift, Side.TOP)
		bottomEdgeEffect = ControlledEdgeEffect(context, shift, Side.BOTTOM)
	}
	...
	private fun getEdgeEffect(side: Side): ControlledEdgeEffect = when (side) {
		Side.TOP -> topEdgeEffect
		Side.BOTTOM -> bottomEdgeEffect
	}

	fun setPullable(side: Side, pullable: Boolean) {
	...
	companion object {
		@JvmStatic
		fun bind(recyclerView: RecyclerView, shift: Shift?): EdgeEffectHandler {
