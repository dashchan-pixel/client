...
class InsetsLayout(context: Context) : FrameLayout(context) {
	class Insets(
	...
	class Apply internal constructor(
		...
		fun get(): Insets {
			...
			return if (bottom != window.bottom)
				Insets(window.left, window.top, window.right, window.bottom) else window
		}
	}
	...
	fun setOnApplyInsetsTarget(view: View?) {
		setOnApplyInsetsListener(if (view != null) OnApplyInsetsListener { applyData ->
	...
	private fun onInsetsChangedInternal(window: Insets, gesture29: Insets, ime29: Insets) {
	...
	override fun onApplyWindowInsets(insets: WindowInsets): WindowInsets {
		try {
			...
			setPadding(0, 0, 0, 0)
			val window: Insets
			val realWindow = Insets(insets.getInsetsIgnoringVisibility(
			...
			window = if (ime29.bottom > realWindow.bottom) {
