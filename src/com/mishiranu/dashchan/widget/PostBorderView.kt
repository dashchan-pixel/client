...
class PostBorderView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
	...
	private fun initializeBorderConfigurationForStyle() {
		borderConfiguration = when (borderStyle) {
			null -> null
	...
	private interface BorderConfiguration {
		fun configure(borderLeft: Int, borderTop: Int, borderRight: Int, borderBottom: Int)

		fun draw(canvas: Canvas)
	}

	private class UserPostBorderConfiguration(context: Context) : BorderConfiguration {
	...
	private class ReplyBorderConfiguration(context: Context) : BorderConfiguration {
		private val paint = Paint(Paint.ANTI_ALIAS_FLAG)
