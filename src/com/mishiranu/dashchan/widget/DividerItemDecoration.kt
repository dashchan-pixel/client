...
class DividerItemDecoration(
    ...
    private fun drawDivider(
        ...
        val boundsTop = if (translate) (top + view.translationY).toInt() else top
        drawable.setBounds(left, boundsTop, right, boundsTop + height)
            ...
            if (position >= 0) {
                ...
                if (configuration.needed) {
                    ...
                    if (position == parent.adapter!!.itemCount - 1) {
                        drawDivider(
                            c,
                            rect.bottom - configuration.bottom,
                            currentLeft,
                            currentRight,
                            height,
                            view,
                            configuration.translateValue,
                        )
                    } else {
                        val toNext = aboveCallback?.shouldPlaceAbove(position + 1) == true
                        val top =
                            if (toNext) {
                                rect.bottom + configuration.top
                            } else {
