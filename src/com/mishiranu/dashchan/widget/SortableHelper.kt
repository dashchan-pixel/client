...
class SortableHelper<VH : RecyclerView.ViewHolder>(recyclerView: RecyclerView,
		private val callback: Callback<VH>) : ItemTouchHelper.Callback() {
	interface Callback<VH : RecyclerView.ViewHolder> {
		fun onDragStart(holder: VH)
		fun onDragFinish(holder: VH?, cancelled: Boolean)
		fun onDragCanMove(fromHolder: VH, toHolder: VH): Boolean
		fun onDragMove(fromHolder: VH, toHolder: VH): Boolean
	}
