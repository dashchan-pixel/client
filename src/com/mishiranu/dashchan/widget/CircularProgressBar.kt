...
class CircularProgressBar(context: Context, attrs: AttributeSet?) : View(context, attrs) {
	...
	private fun getValue(currentTime: Long, fromTime: Long, max: Float): Float {
		return ((currentTime - fromTime) % max) / max
	}

	private var queuedVisible = false

	private val setVisibleRunnable = Runnable {
		visible = queuedVisible
		timeVisibilitySet = SystemClock.elapsedRealtime()
		invalidate()
	}

	fun setVisible(visible: Boolean, forced: Boolean) {
	...
	private fun calculateLollipopTransient(arcStart: Float, arcLength: Float,
			desiredStart: Float, desiredLength: Float, interval: Long): Boolean {
		...
		arcStart += (desiredStart - arcStart) * value
		arcLength += (desiredLength - arcLength) * value
		circularData[0] = arcStart % 1f
		circularData[1] = arcLength
		return finished
	}
	...
	fun setProgress(progress: Int, max: Int, ignoreTransient: Boolean) {
	...
	private fun drawArc(canvas: Canvas, paint: Paint, start: Float, length: Float) {
		var length = length
		if (length < 0.001f) {
			length = 0.001f
		}
	...
	override fun onDraw(canvas: Canvas) {
		...
		var visibilityValue = if (transientVisibility) (time - timeVisibilitySet).toFloat() /
				VISIBILITY_TRANSIENT_TIME else 1f
