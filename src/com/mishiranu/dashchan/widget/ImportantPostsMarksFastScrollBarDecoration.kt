...
class ImportantPostsMarksFastScrollBarDecoration(context: Context) {
	...
	fun draw(scrollBarLeft: Int, scrollBarTop: Int, scrollBarRight: Int, scrollBarBottom: Int, canvas: Canvas) {
		val data = data
	...
	private fun drawPostMarks(postPositions: List<Int>, postMarkPaint: Paint, postMarkRealHeight: Float,
			scrollBarLeft: Int, scrollBarTop: Int, scrollBarRight: Int, canvas: Canvas) {
		...
		while (postPositionIndex <= lastPostPositionIndex) {
			...
			canvas.drawRect(scrollBarLeft.toFloat(), postMarkTop, scrollBarRight.toFloat(),
					postMarkBottom, postMarkPaint)
		}
	}

	class Data(userPostsPositions: Set<Int>, repliesPositions: Set<Int>, @JvmField val totalPostsCount: Int) {
