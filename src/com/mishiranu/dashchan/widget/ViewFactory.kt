...
object ViewFactory {
	...
	fun makeSingleLineListItem(parent: ViewGroup): View {
		...
		val typedArray = textView.context.obtainStyledAttributes(intArrayOf(android.R.attr.textAppearanceListItem,
	...
	private val ATTRS_TWO_LINES = intArrayOf(android.R.attr.listPreferredItemHeightSmall,
			android.R.attr.textAppearanceListItem, android.R.attr.textAppearanceListItemSecondary,
			android.R.attr.textColorSecondary)

	@JvmStatic
	fun makeTwoLinesListItem(parent: ViewGroup, features: Int): TwoLinesViewHolder {
		val density = ResourceUtils.obtainDensity(parent)
		val outerLayout = LinearLayout(parent.context)
		outerLayout.layoutParams = ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
				ViewGroup.LayoutParams.WRAP_CONTENT)
	...
	class ToolbarHolder internal constructor(
		...
		fun update(title: CharSequence?, subtitle: CharSequence?) {
	...
	class ErrorHolder(@JvmField val layout: View, @JvmField val text: TextView)
	...
	fun createErrorLayout(parent: ViewGroup): ErrorHolder {
		...
		image.imageTintList = ResourceUtils.getColorStateList(image.context,
				android.R.attr.textColorSecondary)
		...
		TextViewCompat.setTextAppearance(text, ResourceUtils.getResourceId(text.context,
	...
	@JvmStatic
	fun createSeekLayout(context: Context, showSwitch: Boolean,
			minValue: Int, maxValue: Int, step: Int, valueFormat: String?): SeekLayoutHolder {
