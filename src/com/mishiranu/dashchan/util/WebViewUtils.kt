...
object WebViewUtils {
	...
	@JvmStatic
	fun setProxy(context: Context, proxyData: HttpClient.ProxyData?, callback: Runnable?) {
		val nonNullCallback = callback ?: Runnable {}
		if (proxyData != null) {
			val uriString = (if (proxyData.socks) "socks" else "http") + "://" +
					proxyData.host + ":" + proxyData.port
			ProxyController.getInstance().setProxyOverride(ProxyConfig.Builder()
