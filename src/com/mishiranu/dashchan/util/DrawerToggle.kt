...
class DrawerToggle(private val activity: Activity, toolbarContext: Context?,
	...
	fun syncState() {
		if (mode != Mode.DISABLED) {
			arrowDrawable.setPosition(if (mode == Mode.UP ||
					drawerLayout.isDrawerOpen(GravityCompat.START)) 1f else 0f)
	...
	private inner class ArrowDrawable(context: Context) : BaseDrawable() {
		...
		override fun draw(canvas: Canvas) {
			...
			path.close()
			canvas.save()
			canvas.rotate(canvasRotate * (if (verticalMirror xor rtl) -1f else 1f),
	...
	private inner class StateArrowAnimatorListener(private val enable: Boolean) :
	...
	private fun isLayoutRtl(): Boolean {
		return activity.window.decorView.layoutDirection == View.LAYOUT_DIRECTION_RTL
	}

	companion object {
		private val DRAWER_CLOSE_DURATION: Int = try {
