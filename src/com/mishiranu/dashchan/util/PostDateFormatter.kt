...
class PostDateFormatter(context: Context) {
	...
	init {
		val systemFormat = android.text.format.DateFormat.getDateFormat(context) as SimpleDateFormat
		...
		val shortDateFormat = if (systemPattern.indexOf('d') > systemPattern.indexOf('M'))
			"$monthFormat$divider$dayFormat" else "$dayFormat$divider$monthFormat"
		val longDateFormat = if (systemPattern.indexOf('d') > systemPattern.indexOf('y'))
			"yy$divider$shortDateFormat" else "$shortDateFormat${divider}yy"
	...
	class Holder internal constructor(@JvmField val text: String, internal val instance: String)
}
