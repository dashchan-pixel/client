...
class HistoryAdapter(context: Context, private val callback: Callback, private val chanName: String?) :
	...
	private enum class Header(val titleResId: Int, val threshold: Long) {
		...
		companion object {
			fun find(dayStart: Long, time: Long): Header {
	...
	private fun copyItem(position: Int): HistoryDatabase.HistoryItem {
		return historyItem.update(moveTo(position)).copy()
	}

	override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
		return ListViewUtils.bind(SimpleViewHolder(ViewFactory.makeTwoLinesListItem(parent,
	...
	fun getItemHeader(context: Context, position: Int): String? {
