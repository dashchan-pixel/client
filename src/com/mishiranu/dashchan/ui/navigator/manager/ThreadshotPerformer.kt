...
class ThreadshotPerformer(
	...
	private val task: ExecutorTask<Void?, InputStream?> = object : ExecutorTask<Void?, InputStream?>() {
		override fun run(): InputStream? {
			val time = SystemClock.elapsedRealtime()
			val configurationSet = UiManager.ConfigurationSet(this@ThreadshotPerformer.chanName, null,
			...
			for (postItem in postItems) {
				val measuredHeight = ConcurrentUtils.mainGet {
	...
	init {
		val context: Context = ThemeEngine.attach(ContextThemeWrapper(
				MainApplication.getInstance().localizedContext, 0))
	...
	private fun start(fragmentManager: FragmentManager, chanName: String, boardName: String?,
	...
	class ThreadshotViewModel : TaskViewModel<ExecutorTask<Void?, InputStream?>, InputStream?>()
}
