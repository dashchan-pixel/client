...
class ThreadsPage :
    ListPage(),
    ThreadsAdapter.Callback,
    FavoritesStorage.Observer,
    UiManager.Observer,
    ReadThreadsTask.Callback {
    ...
    private val postStateProvider =
        object : UiManager.PostStateProvider {
    ...
    override fun onCreate() {
        ...
        if (initRequest.errorItem != null) {
            ...
            if (!initRequest.shouldLoad && retainableExtra.cachedPostItems.isNotEmpty()) {
                load = false
                adapter.setItems(
                    retainableExtra.cachedPostItems,
            ...
            if (readViewModel.hasTaskOrValue()) {
                ...
                retainableExtra.startPageNumber =
                    if (board.allowCatalog && Preferences.isLoadCatalog(chan)) {
                        PAGE_NUMBER_CATALOG
                    } else {
                        0
                    }
    ...
    private fun setupSwipeToHideThread(recyclerView: RecyclerView) {
        val callback =
            object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
                ...
                private fun setOpacityForSwipedThread(
                    swipedThreadViewWidth: RecyclerView.ViewHolder,
                    swipeDeltaX: Float,
                ) {
                ...
                private fun calculateOpacityForSwipedThread(
                    swipedThreadViewWidth: Int,
                    swipeDeltaX: Float,
                ): Float = (1 - 1.5 * (abs(swipeDeltaX) / swipedThreadViewWidth)).toFloat()
    ...
    override fun onHandleNewPostDataList() {
        val page = getPage()
        val newPostData =
            PostingService.consumeNewThreadData(
                context,
                page.chanName,
                page.boardName,
            )
        ...
        if (retainableExtra.startPageNumber > 0) {
            ...
            subtitle =
                resources.getQuantityString(
                    R.plurals.number_posts_per_hour__format,
                    retainableExtra.boardSpeed,
                    retainableExtra.boardSpeed,
                )
        }
    ...
    private fun setThreadHideState(
        postItem: PostItem,
        hideState: PostItem.HideState,
    ) {
        val retainableExtra = getRetainableExtra(RetainableExtra.FACTORY)
        retainableExtra.hiddenThreads.set(postItem.getThreadNumber()!!, hideState)
        CommonDatabase.getInstance().threads.setFlagsAsync(
            getPage().chanName!!,
    ...
    override fun onListPulled(
        ...
        val retainableExtra = getRetainableExtra(RetainableExtra.FACTORY)
        refreshThreads(
            if (getAdapter().isRealEmpty || retainableExtra.startPageNumber == PAGE_NUMBER_CATALOG) {
                RefreshPage.CURRENT
            } else if (side == PullableWrapper.Side.BOTTOM) {
                RefreshPage.NEXT
            } else {
                RefreshPage.PREVIOUS
            },
            true,
        )
    }
    ...
    private fun refreshThreads(
        ...
        if (refreshPage == RefreshPage.CATALOG ||
            ...
            if (pageByPage) {
                ...
                pageNumber =
                    if (refreshPage == RefreshPage.NEXT && currentPageNumber >= 0) {
                        currentPageNumber + 1
                    } else {
                        0
                    }
    ...
    private fun loadThreadsPage(
        ...
        if (!pageNumberExists(pageNumber)) {
            ...
            val validator =
                if (!append &&
                    retainableExtra.cachedPostItems.size == 1 &&
                    retainableExtra.startPageNumber == pageNumber
                ) {
                    retainableExtra.validator
                } else {
                    null
                }
            val task =
    ...
    override fun onReadThreadsSuccess(
    ...
    override fun onReadThreadsFail(
        errorItem: ErrorItem?,
        pageNumber: Int,
    ) {
        getRecyclerView().pullable.cancelBusyState()
    ...
    companion object {
            ...
            InstanceDialog(fragmentManager, null) { provider ->
                ...
                val posting =
                    if (board.allowPosting) {
                        chan.configuration.safe().obtainPosting(boardName, true)
                    } else {
                        null
                    }
                if (posting != null) {
                    ...
                    if (bumpLimit != ChanConfiguration.BUMP_LIMIT_INVALID) {
                        layout.add(
                            context.getString(R.string.bump_limit),
                            context.resources
        ...
        private fun showRedirectDialog(
            fragmentManager: FragmentManager,
            target: RedirectException.Target,
        ) {
            ...
            InstanceDialog(fragmentManager, tag) { provider ->
                ...
                val message =
                    provider.context.getString(
                        R.string.open_forum__format_sentence,
