...
class ArchivePage : ListPage(), ArchiveAdapter.Callback, ReadThreadSummariesTask.Callback {
	...
	private fun refreshThreads(showPull: Boolean, nextPage: Boolean) {
		val page = getPage()
		var pageNumber = 0
		if (nextPage) {
			val retainableExtra = getRetainableExtra(RetainableExtra.FACTORY)
	...
	override fun onReadThreadSummariesSuccess(threadSummaries: List<ThreadSummary>, pageNumber: Int) {
		...
		if (pageNumber == 0 && threadSummaries.isEmpty()) {
			...
			if (pageNumber == 0) {
				...
				val concatenated = ReadThreadSummariesTask
