...
class SavedPageItem(@JvmField val stackItem: StackItem?, @JvmField val createdRealtime: Long,
		@JvmField val threadTitle: String?, @JvmField val allowReturn: Boolean) : Parcelable {
	private fun createInternal(page: Page?): Pair<PageFragment, PageItem> {
		val fragment = stackItem!!.create(if (page == null) null else
	...
	fun create(): Pair<PageFragment, PageItem> {
		return createInternal(null)
	}

	fun createWithNewPage(page: Page): Pair<PageFragment, PageItem> {
	...
	companion object {
		@JvmField
		val CREATOR: Parcelable.Creator<SavedPageItem> = object : Parcelable.Creator<SavedPageItem> {
			override fun createFromParcel(source: Parcel): SavedPageItem {
				val stackItem = if (source.readByte().toInt() != 0)
					StackItem.CREATOR.createFromParcel(source) else null
