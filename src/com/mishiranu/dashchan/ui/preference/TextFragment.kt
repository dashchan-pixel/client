...
class TextFragment : BaseListFragment {
    ...
    override fun onViewCreated(
        ...
        when (Type.valueOf(requireArguments().getString(EXTRA_TYPE)!!)) {
            ...
            Type.CHANGELOG -> {
                ...
                changelogEntries =
                    if (savedInstanceState != null) {
                        BundleCompat.getParcelableArrayList(
                            savedInstanceState,
                            EXTRA_CHANGELOG_ENTRIES,
                            ReadChangelogTask.Entry::class.java,
                        )
                    } else {
                        null
                    }
                errorItem =
                    if (savedInstanceState != null) {
                        BundleCompat.getParcelable(savedInstanceState, EXTRA_ERROR_ITEM, ErrorItem::class.java)
                    } else {
                        null
                    }
                ...
                if (errorItem != null) {
                    ...
                    viewModel.observe(viewLifecycleOwner) { entries, newErrorItem ->
                        ...
                        if (entries != null) {
                            ...
                            val error = newErrorItem ?: ErrorItem(ErrorItem.Type.UNKNOWN)
    ...
    private class ListHeaderSpan(
        sub: Boolean,
    ) : RelativeSizeSpan((if (sub) 12f else 16f) / 14f)

    private class TextAdapter : RecyclerView.Adapter<TextAdapter.ViewHolder>() {
        private class ViewHolder(
            parent: ViewGroup,
        ) : RecyclerView.ViewHolder(PostLinearLayout(parent.context)),
            ListViewUtils.ClickCallback<Unit, ViewHolder> {
    ...
    private class PrefixSpan :
        ...
        override fun getSize(
            ...
            return (
                workPaint.measureText(drawText, 0, drawText.length) +
    ...
    companion object {
        ...
        private fun formatChangelogEntries(
            context: Context,
            changelogEntries: List<ReadChangelogTask.Entry>,
        ): List<CharSequence> {
            val dateFormat =
