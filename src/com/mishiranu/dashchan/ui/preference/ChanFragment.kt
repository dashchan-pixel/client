...
class ChanFragment :
    ...
    override fun onViewCreated(
        ...
        if (!chan.configuration.getOption(ChanConfiguration.OPTION_SINGLE_BOARD_MODE)) {
            addEdit(
                Preferences.KEY_DEFAULT_BOARD_NAME.bind(chanName),
                null,
        ...
        if (board.allowCatalog) {
            addCheck(
                true,
                Preferences.KEY_LOAD_CATALOG.bind(chanName),
        ...
        if (deleting != null && deleting.password) {
            ...
            addEdit(
                Preferences.KEY_PASSWORD.bind(chanName),
                null,
                R.string.password_for_removal,
                R.string.password_for_removal__summary,
        ...
        if (captchaTypes != null && captchaTypes.size > 1) {
            addList(
                Preferences.KEY_CAPTCHA.bind(chanName),
                Preferences.getCaptchaTypeValues(captchaTypes),
                Preferences.getCaptchaTypeDefaultValue(chan),
                R.string.captcha_type,
        ...
        if (!localMode) {
            ...
            if (anotherDomainMode) {
                ...
                values[0] = ""
                val domainPreference =
                    addList(
                        Preferences.KEY_DOMAIN.bind(chanName),
                        values,
                        values[0],
        ...
        if (httpsConfigurable) {
            addCheck(
                true,
                Preferences.KEY_USE_HTTPS.bind(chanName),
        ...
        if (!localMode) {
            val proxyPreference =
                addMultipleEdit<Map<String, String>>(
        ...
        if (aiAgentsPostingSupport) {
            ...
            addCheck(
                true,
                Preferences.KEY_HIDE_AI_POSTS.bind(chanName),
                Preferences.DEFAULT_HIDE_AI_POSTS,
                R.string.hide_ai_posts,
                0,
            )
        }
    ...
    private fun addAnotherDomainPreference(primaryDomain: String): Preference<String> {
        val preference =
            addEdit(
                Preferences.KEY_DOMAIN.bind(getChanName()),
                "",
                R.string.domain_name,
                primaryDomain,
        ...
        private fun initializeDialog() {
            ...
            viewModel.observe(this) { result ->
                ...
                if (result === CheckAuthorizationTask.SUCCESS) {
                    ...
                    val preference =
                        when (AuthorizationType.valueOf(requireArguments().getString(EXTRA_AUTHORIZATION_TYPE)!!)) {
        ...
        override fun run(holder: HttpHolder): ErrorItem {
            try {
                    ...
                    chan.performer
                        ...
                        .onCheckAuthorization(
                            ChanPerformer.CheckAuthorizationData(
                                type,
                                @Suppress("UNCHECKED_CAST") (authorizationData?.toTypedArray() as Array<String?>?),
                                holder,
                            ),
                        )
                return if (result != null && result.success) {
                    ...
                    ErrorItem(ErrorItem.Type.INVALID_AUTHORIZATION_DATA)
                }
