...
class ReencodingDialog : DialogFragment(), DialogInterface.OnClickListener,
	...
	override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
		...
		scrollView.addView(linearLayout, FrameLayout.LayoutParams.MATCH_PARENT,
				FrameLayout.LayoutParams.WRAP_CONTENT)
		return AlertDialog.Builder(context).setTitle(R.string.reencode_image)
	...
	override fun onClick(dialog: DialogInterface, which: Int) {
		...
		for (i in IDS.indices) {
			if (IDS[i] == id) {
				format = if (i == JPEG_POSITION && jpegToJpg.isChecked) {
	...
	companion object {
		...
		private val OPTIONS = arrayOf(GraphicsUtils.Reencoding.FORMAT_JPEG.uppercase(Locale.US),
				GraphicsUtils.Reencoding.FORMAT_PNG.uppercase(Locale.US))
		private val FORMATS = arrayOf(GraphicsUtils.Reencoding.FORMAT_JPEG,
