...
class CommentEditWatcher(private var postingConfiguration: ChanConfiguration.Posting?,
		private val commentView: EditText, private val remainingCharacters: TextView,
		private val layoutCallback: Runnable, private val storeDraftCallback: Runnable) : TextWatcher {
	...
	override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
		...
		if (this.error != error || remainingCharacters.text.length == 0) {
			val color = ResourceUtils.getColor(commentView.context, if (error) R.attr.colorTextError
