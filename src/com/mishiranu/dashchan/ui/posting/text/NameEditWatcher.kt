...
class NameEditWatcher(
    private val watchTripcodeWarning: Boolean,
    private val nameView: EditText,
    private val tripcodeWarning: TextView,
    ...
    override fun afterTextChanged(s: Editable) {
        ...
        if (index >= 0) {
            val tripcodeSpan =
                tripcodeSpan ?: ForegroundColorSpan(
                    if (watchTripcodeWarning) {
                        ResourceUtils.getColor(nameView.context, R.attr.colorTextError)
                    } else {
                        ThemeEngine.getTheme(nameView.context).tripcode
                    },
                ).also { tripcodeSpan = it }
