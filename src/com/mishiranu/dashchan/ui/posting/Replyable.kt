...
fun interface Replyable {
	...
	class ReplyData(@JvmField val postNumber: PostNumber?, @JvmField val comment: String?) : Parcelable {
		...
		companion object {
			@JvmField
			val CREATOR: Parcelable.Creator<ReplyData> = object : Parcelable.Creator<ReplyData> {
				override fun createFromParcel(source: Parcel): ReplyData {
					val postNumber = if (source.readByte().toInt() != 0)
						PostNumber.CREATOR.createFromParcel(source) else null
