...
class DialogMenu(
    ...
    fun add(
        titleRes: Int,
        runnable: Runnable,
    ): DialogMenu = add(ViewType.ITEM, context.getString(titleRes), false, runnable)

    fun add(
        title: String,
        runnable: Runnable,
    ): DialogMenu = add(ViewType.ITEM, title, false, runnable)

    fun addMore(
        titleRes: Int,
        runnable: Runnable,
    ): DialogMenu = add(ViewType.MORE, context.getString(titleRes), false, runnable)
    ...
    private class Adapter(
        ...
        ListViewUtils.ClickCallback<ListItem, ViewHolder> {
        private val layoutResId =
            ResourceUtils.obtainAlertDialogLayoutResId(
                context,
                ResourceUtils.DialogLayout.SIMPLE,
            )

        override fun getItemViewType(position: Int): Int = listItems[position].viewType.ordinal
        ...
        init {
            if (viewType != ViewType.ITEM) {
                ...
                if (viewType == ViewType.MORE) {
                    ...
                    val typedArray =
                        parent.context.obtainStyledAttributes(
                            null,
                            attrs,
                            android.R.attr.listMenuViewStyle,
                            0,
                        )
                    try {
    ...
    private class SubMenuArrowDrawable(
        view: View,
    ) : BaseDrawable() {
