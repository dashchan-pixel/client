...
- `content/net/Proxy*.kt` — the proxy provider. `ProxyProvider` is the façade and the shared half; `ProxyService` is the seam, and each service is one object in one file (`AsocksService`, `DataImpulseService`). **Adding a service is a new file plus one entry in `ProxyProvider.SERVICES`** — nothing else counts them, and the settings dropdown is built off that list. The `ProxyService` KDoc is the contract: what `assign`/`rotate` must do, what the shared half hands you, and why a service can't have settings fields of its own. Asocks is the default (and the first entry, which is what a configuration written before the choice existed reads as); **DataImpulse is a kept backup, not a recommendation** — it wants a 50 USD top-up once the trial plan is spent.
