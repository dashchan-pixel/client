...
- `ui/posting/PostingFragment.kt` + `widget/PostingSheetLayout.kt` — the submission form has two hosts (opt-in `Preferences.isPostingSheet`): the content fragment it has always been, *replacing* the page, or a bottom sheet **added over** that page so the thread can be read behind it. Which one it is comes from the activity (`PostingFragment.sheet`, set in `MainActivity.navigatePostingSheet`), never from the fragment reading the setting — a form restored from the fragment stack is hosted plainly and must not think it is still a sheet. Two things follow from a sheet being a second fragment in `content_fragment`: `MainActivity.currentFragment` (topmost) is the sheet while `screenFragment` is the page under it, and **everything a page publishes to the activity must go through `screenFragment`** or it will cast the sheet to `PageFragment`; and the sheet owns no toolbar — no title, no menu item, its markup bar inline and its attach action in the sheet's own header row. `PostingSheetLayout` is deliberately *not* an `ExpandedScreen.Layout`: taking the insets as margins is what makes the sheet exactly as tall as the space below the toolbar, and a sheet inset by `expandedOffset` inside a full-height parent instead hangs that far off the bottom of the screen, taking the send button with it. A sheet also only drags by its content when that content opts into nested scrolling, which a plain `ScrollView` does not do by itself.
